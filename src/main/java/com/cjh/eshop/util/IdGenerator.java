...
public class IdGenerator {
	...
	// 根据主机id获取机器码
	private static IdGenerator worker = new IdGenerator();
	...
	public IdGenerator() {
		this.workerId = getAddress() % (IdGenerator.maxWorkerId + 1);
	}
	...
	public synchronized long nextId() {
		...
		long nextId = ((timestamp - twepoch << timestampLeftShift))
				| (this.workerId << IdGenerator.workerIdShift)
				| (this.sequence);
	...
	private static long getAddress() {
		try {
			String currentIpAddress = InetAddress.getLocalHost()
					.getHostAddress();
